...
export class MintSeed extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save MintSeed entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type MintSeed must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("MintSeed", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): MintSeed | null {
    return changetype<MintSeed | null>(
      store.get_in_block("MintSeed", id.toHexString())
    );
  }

  static load(id: Bytes): MintSeed | null {
    return changetype<MintSeed | null>(store.get("MintSeed", id.toHexString()));
  }
